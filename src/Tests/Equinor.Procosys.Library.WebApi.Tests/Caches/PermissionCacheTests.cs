...
namespace Equinor.Procosys.Library.WebApi.Tests.Caches
{
    ...
    public class PermissionCacheTests
    {
        ...
        private readonly Guid _oid = new Guid("{3BFB54C7-91E2-422E-833F-951AD07FE37F}");
        ...
        public void Setup()
        {
            ...
            _permissionApiServiceMock.Setup(p => p.GetPermissionsAsync(_testPlant))
                .Returns(Task.FromResult<IList<string>>(new List<string> { _permission1, _permission2 }));
        ...
        {
            // Act
            var result = await _dut.GetPermissionsForUserAsync(_testPlant, _oid);

            // Assert
            AssertPermissions(result);
            _permissionApiServiceMock.Verify(p => p.GetPermissionsAsync(_testPlant), Times.Once);
        }

        [TestMethod]
        public async Task GetPermissionsForUserOid_ShouldReturnPermissionsFromCacheSecondTime()
        {
            await _dut.GetPermissionsForUserAsync(_testPlant, _oid);
            // Act
            var result = await _dut.GetPermissionsForUserAsync(_testPlant, _oid);
